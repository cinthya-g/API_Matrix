...
/**
 * Calcula la matriz inversa de una solo objeto Matrix.
 * MatrixInverse  extiende a la clase UnaryOperator
 * (Multiplicación del inverso del determinante por la matriz
 * ajunta transpuesta)
 * @author Cinthya G, Iker J, Valeria R
 *
 */
public class MatrixInverse extends UnaryOperator {
	...
	public Matrix calc(Matrix m) throws NegativeNumberFoundException {		
        ...
        Matrix matrix = getAdjoint(m);
        Matrix solve = new Matrix(m.getRow(),m.getColumn());

        for (int row = 0; row < m.getRow(); row++) {
            for (int col = 0; col < m.getColumn(); col++) {           	
                double str = matrix.getValue(row, col) / det;
                solve.setValue(row, col, str);
            }
        }        
        return solve;
	 ...
	 */
	private Matrix getAdjoint(Matrix m) throws NegativeNumberFoundException {
